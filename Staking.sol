...
contract Staking is ReentrancyGuard {
    ...
    modifier updateReward(address account) {
        s_rewardPerTokenStored = rewardPerToken(); //10
        s_lastUpdateTime = block.timestamp; //10
        s_rewards[account] = earned(account);
        s_userRewardPerTokenPaid[account] = s_rewardPerTokenStored; //10
        _;
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            ...
            (((block.timestamp - s_lastUpdateTime) * REWARD_RATE * 1e18) / s_totalSupply); //100.000.000 sup
    }
